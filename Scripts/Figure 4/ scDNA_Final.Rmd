...
```{r celltypes}
library(cellpypes)
seurat <- pype_from_seurat(
    raw      =SeuratObject::GetAssayData(s, "counts"),
    neighbors=as(s@graphs[["Protein_snn"]], "dgCMatrix")>.1, # sometims "wknn"
    embed    =FetchData(s, vars=c("UMAP_1", "UMAP_2")),
    totalUMI = s$nCount_Protein
)
...
plot_classes(pype)+
  ggtitle("PBMCs annotated with cellpypes")
```
